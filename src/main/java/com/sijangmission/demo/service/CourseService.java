...
public class CourseService {
    ...
    public void deleteCourse(Long courseId) {
		...
		if (recommendedCourse.isEmpty()) {
			// AI가 잘못된 ID를 반환한 경우 첫 번째 코스 사용
			finalCourse = courses.get(0);
		} else {
			finalCourse = recommendedCourse.get();
		}
		...
		return CourseRecommendationResponse.builder()
				...
				.recommendationReason("사용자가 선택한 키워드: " + String.join(", ", request.getTags()) + "와 가장 적합한 코스입니다.")
				.confidenceScore(0.85) // 임시 신뢰도 점수
				.build();
	}
}
